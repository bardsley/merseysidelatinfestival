...
export default function TicketList() {
  ...
  const sendTicketsToSelected = () => {
    // Implement the logic to send tickets to selected attendees
    console.log("Sending tickets to:", selectedTickets);
    const apiResponse = fetch('/api/admin/attendees/email', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ emails: selectedTickets })
    })
  ...
  // else if (isValidating) { return <p>Validating...plz</p>} 
    ...
    return (
      <div className="px-0 my-8 ">
        ...
        { activeTicket && !nameChangeModalActive && !ticketTransferModalActive ? <div className='fixed z-50 w-full'>
        ...
        {JSON.stringify(selectedTickets)}<br/>
        {selectedTickets.length>0 ?
        <button className='bg-chillired-400 text-white rounded-full px-4 py-2 mb-4 hover:bg-chillired-600 transition-colors' onClick={sendTicketsToSelected}>
          Send Tickets to Select
        </button> : null }
        ...
        <div className="-mx-4 sm:mx-0 mt-3 ">
          <table className="min-w-full">
            <thead className="bg-richblack-700">
              <tr className=''>
                {authUsage(user,'multi') ? <th scope="col" className={`${headerClassNames} sm:rounded-l-lg text-center`}><input type="checkbox" /></th> : null}
            ...
            <tbody className="divide-y divide-gray-800 bg-none">
              ...
              <TicketRow key={attendee.ticket_number} attendee={attendee} setActiveTicket={setActiveTicket} setNameChangeModalActive={setNameChangeModalActive} setTicketTransferModalActive={setTicketTransferModalActive} withCheckbox={authUsage(user,'multi')} multiSelectFunc={setSelectedTickets}/>
              )}
            </tbody>
          </table>
